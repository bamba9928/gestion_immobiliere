...
from .forms import (
    BienForm, InterventionForm, ContactSiteForm, ContactAnnonceForm,
    BailForm, EtatDesLieuxForm, LocataireCreationForm, DepenseForm, UnifiedCreationForm
)
# CORRECTION 1 : Retrait de UserProfile de cet import car il est dans accounts
from .models import (
    Bien, Bail, Loyer, Intervention, Annonce, EtatDesLieux, Transaction, Depense
)
from .permissions import (
    is_admin, is_bailleur, is_locataire, get_active_bail
)
...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            "villes": base_qs.values_list("bien__ville", flat=True).distinct().order_by("bien__ville"),
...
class AnnonceDetailView(FormMixin, DetailView):
    ...
    def get_queryset(self):
        return Annonce.objects.filter(statut="PUBLIE", bien__in=Bien.objects.disponibles()).select_related("bien",
                                                                                                           "bien__proprietaire")
...
def dashboard(request):
    ...
    if is_admin(request.user):
        ...
        context["bail"] = bail
        context["has_active_bail"] = bool(bail)
        if bail:
            context["loyers"] = Loyer.objects.filter(bail=bail).order_by("-date_echeance")
            context["dernier_loyer"] = context["loyers"].first()
            context["interventions_recents"] = Intervention.objects.filter(bien=bail.bien,
                                                                           locataire=request.user).select_related(
                "bien").order_by("-created_at")[:5]
        else:
            context["loyers"] = Loyer.objects.none()
            context["dernier_loyer"] = None
            context["interventions_recents"] = Intervention.objects.none()
    else:
...
def grand_livre(request):
    ...
    recettes = Transaction.objects.filter(est_validee=True, created_at__year=annee).select_related("loyer__bail__bien",
                                                                                                   "loyer__bail__locataire")
...
def export_grand_livre_excel(request):
    ...
    for d in depenses:
        ws.append(
            [d.date_paiement, "DEPENSE", f"{d.get_type_depense_display()} : {d.libelle}", d.bien.titre, 0, d.montant])
...
def gestion_bien_detail(request, pk):
    ...
    bail_actif = bien.baux.filter(est_signe=True, date_debut__lte=date.today(), date_fin__gte=date.today()).order_by(
        "-date_debut").first()
    interventions = bien.interventions.select_related("locataire").order_by("-created_at")[:5]

    total_recettes = Transaction.objects.filter(loyer__bail__bien=bien, est_validee=True).aggregate(Sum("montant"))[
                         "montant__sum"] or 0
    total_depenses = Depense.objects.filter(bien=bien).aggregate(Sum("montant"))["montant__sum"] or 0
    cash_flow = total_recettes - total_depenses

    return render(request, "biens/gestion_detail.html", {
        "bien": bien,
...
def add_bail(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                logger.error(f"Erreur génération contrat PDF bail {bail.id}: {e}")
    ...
    return render(request, "baux/add_bail.html", {
        "form": form,
        "user_role": "ADMIN" if is_admin(request.user) else "BAILLEUR"
    })
...
def bail_detail(request, pk):
    ...
    user_role = "ADMIN" if is_admin(request.user) else (
        "BAILLEUR" if bail.bien.proprietaire == request.user else "LOCATAIRE")

    return render(request, "baux/bail_detail.html", {
        "bail": bail,
        "loyers": loyers,
        "etats": etats,
...
def download_contrat(request, bail_id):
    """Téléchargement sécurisé du contrat"""
    bail = get_object_or_404(Bail, pk=bail_id)
    ...
    if not bail.fichier_contrat:
        messages.error(request, "Aucun contrat signé n'est disponible pour ce bail.")
        return redirect("bail_detail", pk=bail.pk)

    response = FileResponse(bail.fichier_contrat.open("rb"), content_type="application/pdf")
    response["Content-Disposition"] = f'inline; filename="Contrat_Location_{bail.id}.pdf"'
    return response


# ============================================================================
# LOYERS, ETATS DES LIEUX, UTILISATEURS
# ============================================================================
...
def loyers_list(request):
    ...
    return render(request, "interventions/loyers_list.html",
                  {"loyers": loyers, "current_statut": statut, "stats": stats})
...
def mark_loyer_paid(request, loyer_id):
    ...
    try:
        ...
        logger.error(f"Erreur marquage loyer {loyer_id}: {e}")
...
def download_quittance(request, loyer_id):
    loyer = get_object_or_404(Loyer.objects.select_related("bail__locataire", "bail__bien__proprietaire"), id=loyer_id)
    ...
    if not loyer.quittance:
        try:
            ...
            logger.error(f"Erreur génération quittance loyer {loyer.id}: {e}")
            messages.error(request, "Impossible de générer la quittance.")
            return redirect("dashboard")

    return FileResponse(loyer.quittance.open("rb"), content_type="application/pdf",
...
def add_etat_des_lieux(request, bail_id, type_edl):
    ...
    if request.method == "POST":
        ...
        # Hack pour les champs désactivés
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                if hasattr(locataire, "profile"):
                    locataire.profile.telephone = cd.get("telephone")
                    locataire.profile.cni_numero = cd.get("cni_numero")
                    locataire.profile.save()

                # 2) Propriétaire
                proprietaire = cd.get("proprietaire") if (
                            is_admin(request.user) and cd.get("proprietaire")) else request.user
...
# ============================================================================
# AUTRES (Dépenses, GED, KYC, Interventions, Actions Admin)
# ============================================================================
...
def documents_list(request):
    ...
    if is_admin(user):
        baux = Bail.objects.select_related("bien", "locataire").filter(fichier_contrat__isnull=False).exclude(
            fichier_contrat="")
        quittances = Loyer.objects.select_related("bail__locataire", "bail__bien").filter(
            quittance__isnull=False).exclude(quittance="")
        ...
        baux = Bail.objects.filter(bien_id__in=biens_ids).select_related("locataire", "bien").filter(
            fichier_contrat__isnull=False).exclude(fichier_contrat="")
        quittances = Loyer.objects.filter(bail__bien_id__in=biens_ids).select_related("bail__locataire",
                                                                                      "bail__bien").filter(
            quittance__isnull=False).exclude(quittance="")
        edls = EtatDesLieux.objects.filter(bail__bien_id__in=biens_ids).select_related("bail__bien").filter(
            pdf__isnull=False).exclude(pdf="")

    elif is_locataire(user):
        baux = Bail.objects.filter(locataire=user).select_related("bien").filter(fichier_contrat__isnull=False).exclude(
            fichier_contrat="")
        quittances = Loyer.objects.filter(bail__locataire=user).select_related("bail__bien").filter(
            quittance__isnull=False).exclude(quittance="")
        edls = EtatDesLieux.objects.filter(bail__locataire=user).select_related("bail__bien").filter(
            pdf__isnull=False).exclude(pdf="")
...
def interventions_list(request):
    ...
    if user_is_admin:
        interventions = Intervention.objects.select_related("bien", "locataire", "bien__proprietaire").order_by(
            "-created_at")
    elif user_is_bailleur:
        interventions = Intervention.objects.filter(bien__proprietaire=request.user).select_related("bien",
                                                                                                    "locataire").order_by(
            "-created_at")
    elif bail:
        interventions = Intervention.objects.filter(bien=bail.bien, locataire=request.user).select_related(
            "bien").order_by("-created_at")
    else:
...
def trigger_rent_generation(request):
    ...
    try:
        ...
        thread.start()
        messages.info(request, "⚙️ La génération des loyers a démarré en arrière-plan.")
    except Exception as e:
...
# ============================================================================
# LISTES ADMIN & CRÉATION RAPIDE
# ============================================================================

# CORRECTION 2 : Ajout de la classe Mixin manquante
...
class AdminBailListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if statut == "actif":
            qs = qs.filter(est_signe=True, date_debut__lte=date.today(), date_fin__gte=date.today())
        return qs
...
class AdminLocataireListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return User.objects.filter(groups__name="LOCATAIRE").select_related("profile").distinct().order_by("last_name")
...
class AdminBailleurListView(AdminRequiredMixin, ListView):
    ...
    def get_queryset(self):
        return User.objects.filter(groups__name="BAILLEUR").select_related("profile").distinct().order_by("last_name")


def _simple_user_add(request, group_name, template):
    if not is_admin(request.user):
        raise PermissionDenied("Accès réservé.")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f"{group_name} créé(e) avec succès.")
            return redirect("dashboard")
    else:
        form = LocataireCreationForm()

    return render(request, template, {"form": form, "user_role": "ADMIN"})


@login_required
def add_locataire(request):
    return _simple_user_add(request, "LOCATAIRE", "utilisateurs/add_locataire.html")


@login_required
def add_bailleur(request):
    return _simple_user_add(request, "BAILLEUR", "utilisateurs/add_bailleur.html")


@login_required
def add_agent(request):
    return _simple_user_add(request, "AGENT", "utilisateurs/add_agent.html")